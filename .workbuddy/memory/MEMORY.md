...
**新增强健（v0.5.0 起）：** 字母键快速定位（case-insensitive 前缀匹配）· **Ctrl+1/2/3** 排序列（名/日期/大小）· **Alt+R** 反转排序方向 · 鼠标点击列表首行排序列头切换排序，同列二次点击反转；双击路径栏段跳转至该级目录。· 鼠标滚轮逐行滚动（不再半屏跳跃）。
...
- **bubbletea ctrl+key 行为**：Windows console 下 ctrl+a/b/c → `KeyCtrlA/B/C`（`\x01/\x02/\x03`），但 ctrl+1/2/3 本身不产生命名 KeyCtrl*，实际以 `KeyRunes{'1'/'2'/'3'}`（ASCII digit 49/50/51）或 `KeyRunes{0x11/0x12/0x13}`（控制字节）到达，取决于终端类型（Windows Terminal vs conhost）。匹配逻辑必须兼容三种形式；`KeyCtrlC` 排除（quit 已在顶部拦截）。
