...
| Batch rename | `F2` (when items selected) | Multi-select, preview, then write |
...
| Delete | `Del` / `F8` | With confirmation prompt |
...
| `F2` | Open batch-rename preview (with selections) |
| `F3` | Open file in associated app (when on a file) / open directory tree overlay (when on a dir, when available) |
| `F4` | Edit file in associated app |
| `F5` | Copy selected item(s) to async queue |
| `F6` | Move selected item(s) to async queue |
| `F7` | Create new directory; `Alt+F7` opens a standalone terminal at the cursor's path and copies the path to clipboard |
| `F8` / `Del` | Delete selected item(s) |
| `Space` | Toggle selection |
| `Ctrl+A` | Select all |
| `Ctrl+E` | Open association editor (Ctrl+E is terminal-dependent; use `:assoc` as fallback) |
| `Ctrl+T` | New tab |
| `Ctrl+W` | Close current tab |
| `Ctrl+C` | Cancel the active operation (copy/move/rename) |
| `/` | Start name-filter search |
| `:` | Start command input (type `assoc` → Enter to open assoc editor; `config` → config editor) |
